...
/* ==========================================================================
   2. SEVEN-AGENT ORCHESTRATOR & GATEWAY ROUTER (Connected to Live Env Server)
   ========================================================================== */

export class AgentOrchestrator {
  constructor(apiKeySarvam, apiKeyGemini) {
    this.activeProviderName = "SBI Saathi Secure Backend Engine";
    this.backendUrl = "http://localhost:5000/api/orchestrate";

    if (apiKeySarvam && apiKeySarvam.trim().length > 0) {
      this.activeProviderName = "Sarvam AI (Indic NLP API)";
    } else if (apiKeyGemini && apiKeyGemini.trim().length > 0) {
      this.activeProviderName = "Google Gemini Pro Gateway";
    }
  }
  ...
  async orchestrate(query, lang) {
    ...
    try {
      ...
      console.warn("Express server unreachable or key limit reached. Engaging Smart Local Fallback.");
      
      // Keep UX alive with low-latency local reasoning core fallback
      ...
      return {
        ...
        language: lang,
        confidence: 0.45,
        nextStep: "Could you rephrase your target goal? Try saying: 'I want to pay my electricity bill'.",
        spotlightTarget: "unknown",
        guidanceText: "I could not resolve an automated path with confidence. Let me know if you would like me to escalate to manual human support.",
        recommendation: "Supported Journeys Overview",
        recommendationDetail: "Check our sandbox components to explore valid interactive journeys.",
        complianceChecked: true
      };
    }
  }
...
const ConversationalAssistant = ({ accessibilityMode, isProcessing, agentMessage }) => (
...
export default function App() {
  // PORTAL SEPARATION & LOGIN STATES
  ...
  // Interactive Voice Recording & Waveform State
  const [isListeningVoice, setIsListeningVoice] = useState(false);
  const [voiceQuery, setVoiceQuery] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);

  // Active virtual assistant guidelines message
  const [agentMessage, setAgentMessage] = useState('Welcome back to your companion. I am Saathi. Just say or type your request, and I will safely pilot you through the correct banking menus.');
  ...
  // Authentication Pin Modal State
  ...
  const addLog = (message) => {
    ...
    setSystemLogs(prev => [`[${timeStr}] ${message}`, ...prev.slice(0, 15)]);
  };
  ...
  }, [isListeningVoice, accessibilityMode]);
  ...
  const handleQueryDispatch = async (rawQuery) => {
    ...
    setAgentMessage('Orchestration pipeline initialized. Activating companion agents to parse request...');
    
    // Reset agent trackers status
    ...
    // Step 4: Safety & Compliance (7th Agent)
    ...
    // Fire actual Core Orchestrator (Connected to Backend)
    const orchestrator = new AgentOrchestrator(sarvamKey, geminiKey);
    const output = await orchestrator.orchestrate(rawQuery, currentLanguage);
    setLastOutput(output);

    // Step 5: Guidance Focus
    ...
    // Handle Confidence Threshold Escalation
    if (output.confidence < 0.70) {
      setAgentMessage('I resolved your request with lower confidence. Would you prefer to modify your statement, or should I escalate you to live branch human support?');
      setAgentThoughts([
    ...
    setAgentThoughts([
      ...
      `Compliance Registry Status: BBPS Stamped.`,
  ...
  // Browser-native speech capturing with fully automated fallbacks
  const startSpeechCapture = () => {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;

    if (!SpeechRecognition) {
      addLog("Local browser lacks Speech Recognition API support. Triggering Speech Emulation Module.");
      runSimulatedSpeech();
      return;
    }
    ...
    try {
      ...
      recognition.lang = currentLanguage === 'hi' ? 'hi-IN' : currentLanguage === 'ta' ? 'ta-IN' : 'en-IN';

      recognition.onstart = () => {
        setIsListeningVoice(true);
        setAgentPhase('listening');
        setAgentMessage('Listening to speech input channel...');
        addLog(`Web Speech API initialized using: ${recognition.lang}`);
      };

      recognition.onresult = (event) => {
        const resultText = event.results[0][0].transcript;
        setVoiceQuery(resultText);
        addLog(`Speech Transcribed Output: "${resultText}"`);
        handleQueryDispatch(resultText);
      };

      recognition.onerror = (err) => {
        addLog(`Speech Recognition API Error: [${err.error}]. Initializing simulator fallback...`);
        runSimulatedSpeech();
      };
      ...
      console.warn("Speech API Start Interrupted:", e);
      runSimulatedSpeech();
    }
  };

  // Fallback simulator to ensure demo never breaks during rate-limiting or insecure origins
  const runSimulatedSpeech = () => {
    setIsListeningVoice(true);
    setAgentPhase('listening');
    setAgentMessage('Microphone channel synchronized. Listening to your voice...');
  ...
  // Simulated bill search & DB verification (Electricity)
  const handleBBPSQueryFetch = (e) => {
    e.preventDefault();
    if (consumerNumber.length !== 11) return;

    setAgentMessage('Connecting to Bharat BillPay registries to check outstanding balances...');
    addLog(`POST /api/navigation -> Querying utilities bill ledger: #${consumerNumber}`);

    setTimeout(() => {
      setBillFetched(true);
      setAgentMessage('Pending invoice loaded successfully. Please review all details and click checkout pay manually.');
      setSpotlightElementId('manual-checkout-btn');
      setSpotlightGuideText('Outstanding bill fetched! Click confirm pay below to authenticate.');
  ...
  // Secure Transaction Executor (Requires authentication MPIN)
  const executeSecureTransaction = (e) => {
    ...
    setTimeout(() => {
      ...
      setAgentMessage('Transaction successful! Money has been transferred securely.');
      setSpotlightElementId(null);
      setCompletedJourneys(prev => [...prev, authModal.journey || '']);
      setDemoMetrics(prev => ({ ...prev, totalCompleted: prev.totalCompleted + 1 }));
      addLog('Security Audit Gate: MPIN signature verified. Funds released.');
    }, 1500);
  };
  ...
  const handleResetWorkflow = () => {
    ...
    setAgentPhase('idle');
    setAgentMessage('Welcome back to your companion companion. I am Saathi. Just say or type your request, and I will safely pilot you through the correct banking menus.');
    setAgentThoughts([]);
    setConsumerNumber('');
    ...
    setAuthModal({ isOpen: false, journey: null });
    setAuthPin('');
    addLog('Sandbox cached parameters cleared.');
  };

  const handleLogin = (e) => {
    e.preventDefault();
    if (username && password) {
      setPortalMode(loginRole || 'user');
      setIsLoggedIn(true);
    }
  };
  ...
  const handleInterceptEscalation = (targetUser) => {
    ...
    addLog(`HITL Intercept: Administrator co-piloting active screen of client "${targetUser}"`);
  };
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-[#F5F3FF] flex items-center justify-center p-4 relative overflow-hidden font-sans">
        ...
        <div className="bg-white/90 backdrop-blur-xl border border-purple-200 p-8 md:p-12 rounded-3xl shadow-2xl max-w-2xl w-full z-10 transition-all duration-500 text-slate-800">
          <div className="text-center mb-10">
            ...
            <p className="text-[#8E24AA] mt-2 font-bold tracking-wide uppercase text-xs">Proactive Agentic AI Banking Companion</p>
          </div>
          ...
          {!loginRole ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <p className="text-xs text-slate-505 mt-2 leading-relaxed font-medium">Access the interactive companion to perform guided digital payments.</p>
              </button>
              ...
              >
                ...
                <p className="text-xs text-slate-505 mt-2 leading-relaxed font-medium">Access administrative metrics, compliance overrides, and live system logs.</p>
              </button>
            </div>
          ) : (
              ...
              <form onSubmit={handleLogin} className="space-y-4 text-left">
                <div>
                  ...
                  <input 
                    ...
                    placeholder={loginRole === 'user' ? "rahul_sharma" : "admin_01"}
  ...
  return (
    <div className={`min-h-screen font-sans antialiased relative pb-16 transition-colors duration-300 ${
      accessibilityMode ? 'bg-white text-[#2E1065] text-lg' : 'bg-[#F4F1FA]'
    }`}>
      ...
      <section className={`py-2 px-4 shadow-sm z-30 relative ${
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2 font-extrabold tracking-widest uppercase text-[10px]">
            ...
            <span>Saathi Guides. You Decide.</span>
          </div>
          <p className="text-[10px] font-medium text-center sm:text-right">
            AI coordinates active input focus areas. Passwords, PIN codes & payouts always require manual customer confirmation.
          </p>
        </div>
      </section>
      ...
      <header className={`sticky top-0 z-40 border-b shadow-xs px-4 py-3.5 backdrop-blur-md ${
        portalMode === 'admin' ? 'bg-white border-slate-200 text-slate-800' :
        accessibilityMode ? 'bg-white border-[#4A148C] text-[#2E1065]' : 'bg-white/90 border-purple-200'
      }`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-md font-black tracking-tight flex items-center gap-1.5">
                SBI Saathi 
              ...
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Judges-Ready Architecture Demo</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1.5 mr-2 ${
                portalMode === 'admin' 
                  ? 'bg-rose-505/10 text-rose-500 hover:bg-rose-500/20 border border-rose-500/20' 
              ...
              <>
                <button
                  onClick={() => {
                    ...
                    addLog(`Accessibility high-contrast rendering toggled: ${!accessibilityMode}`);
                  }}
                  className={`px-3 py-1.5 text-[10px] font-extrabold uppercase rounded-lg border tracking-wider transition-all flex items-center gap-1 ${
                    accessibilityMode 
                  ...
                  {['en', 'hi', 'ta'].map(lang => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        addLog(`System transcribing mode switched to: ${lang.toUpperCase()}`);
                      }}
                      className={`px-2 py-1 text-[10px] font-bold rounded-md transition-all ${
                        currentLanguage === lang 
                          ? 'bg-[#4A148C] text-white' : 'text-slate-500 hover:text-slate-800'
                          }`}
                    >
                      {lang.toUpperCase()}
                    </button>
                ...
                <button 
                  ...
                  className={`p-2 rounded-xl border transition-all ${accessibilityMode ? 'bg-purple-100 border-slate-700 text-[#4A148C]' : 'bg-white hover:bg-purple-50 text-[#4A148C] border-purple-200'}`}
                  title="Reset State"
                >
      ...
      {/* ────────────────────────────────────────────────────────
         CUSTOMER VIEWPORT (Clean Centered Layout)
         ──────────────────────────────────────────────────────── */}
      {portalMode === 'user' && (
        <main className="max-w-4xl mx-auto px-4 py-8 flex flex-col gap-6">
          
          {/* Unrecognized query fallbacks */}
          {lastOutput && lastOutput.confidence < 0.70 && !isEscalated && (
            <div className={`p-5 rounded-3xl border-[2px] ${
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider">Uncertain Statement Recognition Escaped</h4>
                  <p className="text-xs mt-1 leading-relaxed">
                    Saathi was unable to map your input to a secure banking transaction route with high certainty. Try one of our pre-built scenarios below, or tap live escalation.
                  </p>
                  ...
                  <div className="flex gap-2.5 mt-3.5">
                    <button
                      ...
                      className={`px-3 py-1.5 text-[10px] font-bold rounded-lg border ${
                        accessibilityMode ? 'bg-white text-rose-700 border-rose-600' : 'bg-white hover:bg-rose-100 border-rose-300'
                      }`}
                    >
                      Pay Electricity
                    </button>
                    <button
                      ...
                      className="px-3 py-1.5 text-[10px] font-bold rounded-lg bg-[#4A148C] text-white"
                    >
          ...
          {isEscalated && (
            <div className={`p-6 rounded-3xl border text-center space-y-4 ${
              accessibilityMode ? 'bg-purple-50 border-[#4A148C]' : 'bg-white border-purple-100 shadow-xl'
            }`}>
              <div className="inline-flex p-3.5 bg-purple-50 rounded-full text-[#4A148C] animate-pulse">
                <PhoneCall className="w-8 h-8" />
              </div>
              <h3 className="text-md font-black text-[#4A148C]">Contacting Active Video Banking Agent</h3>
              <p className="text-xs text-slate-500 leading-relaxed max-w-sm mx-auto">
                We are assigning a live companion support agent to co-pilot your screen session. Standby securely.
              </p>
              <button 
          ...
          {currentScreen === 'landing' && !isEscalated && (
            <div className={`rounded-3xl p-6 lg:p-8 border shadow-xs relative overflow-hidden transition-all ${
              accessibilityMode 
                ? 'bg-white border-[3px] border-[#4A148C]' 
                : 'bg-gradient-to-br from-white to-[#F9F7FC] border-purple-100/50'
            }`}>
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#EC407A]/5 rounded-full filter blur-3xl pointer-events-none" />
              
              <div className="w-full">
                <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full w-fit ${
                  accessibilityMode ? 'bg-[#4A148C] text-white' : 'bg-pink-100/60 text-[#EC407A]'
                }`}>
                  <Award className="w-3.5 h-3.5" /> High-Fidelity Agentic Experience
                </div>
                
                <h2 className="text-3xl font-black mt-4 leading-tight tracking-tight">
                  {accessibilityMode ? "SIMPLE ASSISTED DIGITAL BANKING" : "Stop Searching Complex Menus."}
                </h2>
                <p className="text-xs text-slate-500 mt-2 leading-relaxed">
                  Avoid hunting for features inside folders. Speak or type your financial target in Hindi, Tamil, or English, and let Saathi manage the coordinates.
                </p>

                {/* Voice dictation pipeline container */}
                <div className={`mt-8 p-4.5 rounded-2xl border shadow-lg relative ${
                  accessibilityMode ? 'bg-purple-50 border-[2px] border-[#4A148C]' : 'bg-white border-purple-100'
                }`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] uppercase font-black tracking-wider text-purple-400">Audio Pipeline Gateway</span>
                    {isListeningVoice && (
                      <span className="text-[9px] bg-rose-500 text-white font-extrabold px-2 py-0.5 rounded-full animate-pulse">
                        Listening...
                      </span>
                    )}
                  </div>
                  ...
                  <div className="relative flex items-center">
                    <input
                      ...
                      placeholder='Try: "I need to pay electricity bill of TPDDL"'
                      className={`w-full border rounded-xl py-3.5 pl-4 pr-24 text-xs font-semibold focus:outline-none focus:ring-2 ${
                        accessibilityMode 
                          ? 'bg-white border-[#4A148C] text-[#2E1065] focus:ring-[#4A148C]' 
                          : 'bg-white border-purple-200 text-[#4A148C] focus:ring-[#4A148C]'
                      }`}
                      onKeyDown={(e) => {
                        ...
                        className={`p-2.5 rounded-lg transition-all border ${
                          isListeningVoice 
                            ...
                            : accessibilityMode 
                              ...
                              : 'bg-slate-100 border-slate-200 text-[#4A148C] hover:bg-slate-200'
                        }`}
                        title="Simulate Speech Dictation Stream"
                      >
                        {isListeningVoice ? <Mic className="w-4.5 h-4.5" /> : <MicOff className="w-4.5 h-4.5" />}
                      </button>
                  ...
                  {/* Sinusoidal Waveform Graphic Area */}
                  {isListeningVoice && (
                    <div className="mt-4 p-3 bg-black rounded-xl border border-slate-800 flex flex-col gap-1.5">
                      <div className="flex justify-between text-[9px] font-mono text-slate-400">
                        <span>Acoustic Frequency Channel synchronized</span>
                        <span>Multi-Lingual Dictation Engine Active</span>
                      </div>
                      <canvas ref={canvasRef} width={400} height={50} className="w-full h-12 rounded animate-pulse" />
                    </div>
                  )}
                </div>
                ...
                <div className="mt-7">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5">⚡ Quick Scenario Injectors</p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {[
                      { query: "I need to pay electricity bill of TPDDL" },
                      { query: "Pay tuition university fee" },
                      { query: "Open fixed deposit" }
                    ].map((scenario, idx) => (
                      <button
                        key={idx}
                        onClick={() => {
                        ...
                        className={`p-3 text-left rounded-xl border transition-all text-[11px] font-bold ${
                          accessibilityMode 
                            ? 'bg-white border-[#4A148C] text-[#2E1065] hover:bg-purple-50' 
                            : 'bg-white border-purple-100 text-[#4A148C] hover:border-purple-300'
                        }`}
                      >
                        "{scenario.query}"
                      </button>
            ...
            <div className={`rounded-3xl p-6 lg:p-8 border shadow-lg relative ${
              ...
              <div className="flex items-center justify-between border-b pb-4 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-xl">Electricity Bill Payment</h3>
                    <p className="text-xs text-slate-400 uppercase">BBPS Secure API Gate</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleBBPSQueryFetch} className="space-y-6">
                <div>
                  <label className="block text-[11px] font-black uppercase tracking-wider mb-1.5">Distribution Utility Provider</label>
                  <select 
                    value={selectedBiller}
                    onChange={(e) => setSelectedBiller(e.target.value)}
                    className={`w-full border rounded-xl px-3.5 py-3 text-xs font-semibold outline-none focus:ring-2 ${
                      accessibilityMode 
                        ? 'bg-white border-[#4A148C] text-[#2E1065]' 
                        : 'bg-slate-50 border-slate-200 text-[#4A148C] focus:ring-[#4A148C]'
                    }`}
                  >
                    {MOCK_BILLERS.map(b => (
                ...
                <div className={`p-2 rounded-2xl transition-all ${spotlightElementId === 'consumer-field' ? 'ring-4 ring-[#EC407A] bg-pink-50/10 z-50 relative pointer-events-auto' : ''}`}>
                  <label className="block text-[11px] font-black uppercase tracking-wider mb-1.5">11-Digit Consumer ID</label>
                  <input
                    id="consumer-field"
                    type="text"
                    maxLength={11}
                    ...
                    onChange={(e) => {
                      ...
                      if (numeric.length === 11) {
                        ...
                        setSpotlightGuideText("Consumer Account ID loaded. Now click 'Verify Outstanding Dues' to fetch current invoice parameters.");
                      }
                    }}
                    className={`w-full border rounded-xl py-3 px-4 text-xs font-semibold focus:outline-none focus:ring-2 ${
                      accessibilityMode 
                        ? 'bg-white border-[#4A148C] text-[#2E1065] focus:ring-[#4A148C]' 
                        : 'bg-slate-50 border-slate-200 text-slate-800 focus:ring-[#4A148C]'
                    }`}
                  />
                </div>

                {!billFetched && (
                  <div className={`p-1 rounded-2xl transition-all ${spotlightElementId === 'query-dues-btn' ? 'ring-4 ring-[#EC407A] z-50 relative pointer-events-auto' : ''}`}>
                    <button
                      id="query-dues-btn"
                      type="submit"
                      disabled={consumerNumber.length !== 11}
                      className={`w-full py-3.5 text-xs font-bold rounded-xl transition-all disabled:opacity-40 ${
                        accessibilityMode ? 'bg-[#4A148C] text-white font-black' : 'bg-gradient-to-r from-[#4A148C] to-[#8E24AA] text-white'
                      }`}
                    >
              ...
              {billFetched && (
                <div className={`mt-8 border rounded-2xl p-4.5 ${
                  accessibilityMode ? 'bg-purple-50 border-[#4A148C]' : 'bg-[#FDFBFD] border-purple-100'
                }`}>
                  <div className="flex justify-between items-center border-b pb-3 mb-4">
                    <span className="text-[10px] font-black uppercase text-slate-400">Bharat BillPay (BBPS) Receipt</span>
                    <span className="bg-emerald-100 text-emerald-800 text-[9px] font-bold px-2.5 py-0.5 rounded-full uppercase border border-emerald-200">Verified</span>
                  </div>

                  <div className="grid grid-cols-2 gap-4 text-xs mb-4">
                    <div>
                      <span className="text-slate-400 block text-[9px]">Consumer Name:</span>
                      <span className="font-bold">Amit Kumar Sharma</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[9px]">Due Grace Date:</span>
                      <span className="font-bold text-red-500">15 July 2026</span>
                    </div>
                  </div>

                  <div className={`rounded-xl p-3 flex justify-between items-center border shadow-xs mb-6 ${
                    accessibilityMode ? 'bg-white border-[#4A148C]' : 'bg-white border-purple-55'
                  }`}>
                    <div>
                      <span className="text-slate-400 block text-[9px]">Calculated Outstanding Dues:</span>
                      <span className="text-2xl font-black text-[#EC407A]">₹{billAmount}.00</span>
                    </div>
                  </div>

                  <div className={`p-1 rounded-2xl transition-all ${spotlightElementId === 'manual-checkout-btn' ? 'ring-4 ring-[#EC407A] z-50 relative pointer-events-auto' : ''}`}>
                    <button
                      id="manual-checkout-btn"
                      onClick={handleManualPaymentSettle}
                      disabled={isPaying}
                      className={`w-full py-3.5 text-xs font-bold rounded-xl shadow-lg transition-all ${
                        accessibilityMode ? 'bg-[#4A148C] text-white font-black' : 'bg-gradient-to-r from-[#4A148C] via-[#8E24AA] to-[#EC407A] text-white'
                      }`}
                    >
                      {isPaying ? 'Authenticating Core Security Gateway...' : 'Confirm Pay & Release Funds'}
                    </button>
                  </div>
                </div>
              )}
          ...
          {currentScreen === 'tuition' && (
            <div className={`rounded-3xl p-6 border shadow-md ${
              accessibilityMode ? 'bg-white border-[3px] border-[#4A148C]' : 'bg-white border-purple-100'
            }`}>
              <div className="flex items-center justify-between border-b pb-4 mb-6">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className={`text-lg font-black ${accessibilityMode ? 'text-[#2E1065]' : 'text-[#4A148C]'}`}>Tuition School Fees</h3>
                    <p className="text-xs text-slate-400">Academic Disbursement Service</p>
                  </div>
                </div>
                <button onClick={handleResetWorkflow} className="p-1.5 text-slate-400 hover:text-slate-850"><X className="w-5 h-5" /></button>
              </div>

              <div className="space-y-4">
                <div>
                  <label className="block text-[11px] font-black uppercase mb-1.5">Registered Institution</label>
                  <select 
                    value={selectedSchool}
                    onChange={(e) => setSelectedSchool(e.target.value)}
                    className={`w-full border rounded-xl px-3.5 py-3 text-xs font-semibold outline-none ${
                      accessibilityMode ? 'bg-white border-[#4A148C] text-[#2E1065]' : 'bg-slate-50 border-slate-200 text-[#4A148C]'
                    }`}
                  >
                    {MOCK_SCHOOLS.map(s => (
                ...
                <div className={`p-2 rounded-2xl ${spotlightElementId === 'student-field' ? 'ring-4 ring-[#EC407A] z-50 relative pointer-events-auto' : ''}`}>
                  <label className="block text-[11px] font-black uppercase mb-1.5">Student Enrollment ID</label>
                  <input
                    id="student-field"
                    type="text"
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value.length > 5) {
                        ...
                        setSpotlightGuideText("Enrollment validated. Verify tuition fee breakdown and manually click the settle button.");
                      }
                    }}
                    className={`w-full border rounded-xl py-3 px-4 text-xs font-semibold focus:outline-none focus:ring-2 ${
                      accessibilityMode 
                        ? 'bg-white border-[#4A148C] text-[#2E1065] focus:ring-[#4A148C]' 
                        : 'bg-slate-50 border-slate-200 text-slate-850 focus:ring-[#4A148C]'
                    }`}
                  />
                </div>

                {studentId.length > 5 && (
                  <div className={`p-4 border rounded-xl flex justify-between items-center text-xs shadow-xs ${
                    accessibilityMode ? 'bg-purple-50 border-[#4A148C]' : 'bg-slate-50 border-purple-50'
                  }`}>
                    <span className="font-bold">Semester Academic Invoice Fees</span>
                    <span className="font-black text-[#EC407A]">₹1,20,000.00</span>
                  </div>
                )}

                <div className={`p-1 rounded-2xl ${spotlightElementId === 'tuition-pay-btn' ? 'ring-4 ring-[#EC407A] z-50 relative pointer-events-auto' : ''}`}>
                  <button
                    id="tuition-pay-btn"
                    disabled={!studentId.trim()}
                    onClick={handleManualTuitionSettle}
                    className={`w-full py-3.5 text-xs font-bold rounded-xl disabled:opacity-40 ${
                      accessibilityMode ? 'bg-[#4A148C] text-white font-black' : 'bg-gradient-to-r from-[#4A148C] to-[#EC407A] text-white'
                    }`}
                  >
                    Confirm & Settle Tuition Fees
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className={`rounded-3xl p-6 border shadow-md ${
              ...
              <div className="flex items-center justify-between border-b pb-4 mb-6">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className={`text-lg font-black ${accessibilityMode ? 'text-[#2E1065]' : 'text-[#4A148C]'}`}>Fixed Deposit Creation</h3>
                    <p className="text-xs text-slate-400">Autonomous Wealth Optimizer</p>
                  </div>
                </div>
                <button onClick={handleResetWorkflow} className="p-1.5 text-slate-400 hover:text-slate-850"><X className="w-5 h-5" /></button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[11px] font-black uppercase mb-1.5">Term Scheme Option</label>
                  <select 
                ...
                <div className={`p-2 -mx-2 rounded-2xl ${spotlightElementId === 'deposit-field' ? 'ring-4 ring-pink-500' : ''}`}>
                  <label className="block px-2 text-xs font-black uppercase mb-1.5">Principal Amount (₹)</label>
                  <input
                    id="deposit-field"
                    type="number"
                    min="10000"
                    ...
                    onChange={(e) => {
                      ...
                      if (parseInt(e.target.value) >= 10000) {
                        ...
                        setSpotlightGuideText("Required principal cleared. Review return yield estimates and manually click the generate button.");
                      }
                    }}
                    className={`w-full border rounded-xl py-3 px-4 text-xs font-semibold focus:outline-none focus:ring-2 ${
                      accessibilityMode 
                  ...
                  />
                  <p className="text-[9px] text-slate-400 mt-1 font-bold">Minimum investment value: ₹10,000</p>
                </div>

                {parseInt(depositAmount) >= 10000 && (
                  <div className={`p-4 border rounded-xl text-xs space-y-1.5 ${
                    accessibilityMode ? 'bg-purple-50 border-[#4A148C]' : 'bg-slate-50 border-purple-55'
                  }`}>
                    <div className="flex justify-between">
                      <span className="font-bold">Interest Rate:</span>
                      <span className="font-bold text-emerald-600">7.10% p.a.</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-bold">Estimated Payout:</span>
                      <span className="font-black text-[#EC407A]">₹{(parseInt(depositAmount) * 1.071).toFixed(2)}</span>
                    </div>
                  </div>
                )}

                <div className={`p-1 rounded-2xl ${spotlightElementId === 'fd-generate-btn' ? 'ring-4 ring-pink-500' : ''}`}>
                  <button
                    id="fd-generate-btn"
          ...
          {currentScreen === 'success' && (
            <div className={`rounded-3xl p-8 border shadow-xl text-center space-y-6 ${
              ...
              <div className="space-y-2">
                <h3 className={`text-2xl font-black ${accessibilityMode ? 'text-[#2E1065]' : 'text-[#4A148C]'}`}>Transaction Processed Safely</h3>
                <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
                  Your payment authorization has been captured, logged, and securely transferred to central clearings.
                </p>
              </div>
              ...
              {lastOutput && lastOutput.recommendation && (
                <div className={`pt-6 border-t text-left ${
                  accessibilityMode ? 'border-[#4A148C]' : 'border-purple-50'
                }`}>
                  <div className="flex justify-between items-center mb-3">
                    <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-full flex items-center gap-1.5 w-fit ${
                      accessibilityMode ? 'bg-[#4A148C] text-white' : 'bg-pink-100/60 text-[#EC407A]'
                    }`}>
                      <Lightbulb className="w-3.5 h-3.5 text-amber-500 animate-pulse" /> Proactive Companion Suggestion
                    </span>
                    <button
                      onClick={() => setExplainModal(true)}
                      className={`text-[9px] font-bold uppercase underline flex items-center gap-0.5 ${
                        accessibilityMode ? 'text-[#4A148C]' : 'text-[#4A148C]'
                      }`}
                    >
                      <Info className="w-3 h-3" /> Why suggest this?
                    </button>
                  </div>

                  <h4 className="text-xs font-black mt-2">{lastOutput.recommendation}</h4>
                  <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">{lastOutput.recommendationDetail}</p>
                </div>
              )}
              ...
              <div className="pt-4">
                <button
                  onClick={handleResetWorkflow}
                  className={`px-6 py-2.5 text-xs font-bold rounded-xl shadow-md ${
                    accessibilityMode ? 'bg-[#4A148C] text-white font-black' : 'bg-gradient-to-r from-[#4A148C] to-[#EC407A] text-white'
                  }`}
                >
                  Return to Dashboard
      ...
      {/* ────────────────────────────────────────────────────────
         ADMIN CONTROL VIEW (2-Column Grid Layout)
         ──────────────────────────────────────────────────────── */}
      {portalMode === 'admin' && (
        <main className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 text-slate-800">
          
          {/* Admin KPI Header Row */}
          <div className="col-span-1 lg:col-span-12 grid grid-cols-2 md:grid-cols-5 gap-4 mb-2">
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col justify-between">
              ...
              <span className="block text-[10px] uppercase font-bold text-slate-505 mt-1">Cleared Sessions</span>
            </div>
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col justify-between">
              <span className="text-3xl font-black text-emerald-600">{(lastOutput ? lastOutput.confidence * 100 : 98).toFixed(0)}%</span>
              <span className="block text-[10px] uppercase font-bold text-slate-505 mt-1">Avg AI Confidence</span>
            </div>
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col justify-between">
              <span className="text-3xl font-black text-[#4A148C]">{demoMetrics.autoPayAdoptions}</span>
              <span className="block text-[10px] uppercase font-bold text-slate-505 mt-1">Proactive Adoptions</span>
            </div>
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden">
              ...
              <span className="block text-[10px] uppercase font-bold text-slate-505 mt-1">AI Latency (Gemini Flash)</span>
            </div>
            <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col justify-between">
              <span className="text-3xl font-black text-rose-500">2</span>
              <span className="block text-[10px] uppercase font-bold text-slate-505 mt-1">Active Escalations</span>
            </div>
          </div>
            ...
            <div className="p-6 rounded-3xl border shadow-sm bg-white border-slate-200 relative overflow-hidden">
              <div className="flex items-center justify-between border-b pb-4 mb-4">
                <h4 className="text-sm font-black uppercase text-slate-800 flex items-center gap-2">
                  <Users className="w-5 h-5 text-[#4A148C]" /> Live Activity Monitor
                </h4>
                <span className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full uppercase">
                  ...
                  <div key={activity.id} className="p-3.5 rounded-xl border border-slate-100 bg-slate-50 flex flex-col gap-2">
                    ...
                    <div className="flex justify-between items-end">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-505 mt-0.5">Value: {activity.value}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {activity.status === 'ESCALATED' ? (
                          <button
                            onClick={() => handleInterceptEscalation(activity.user)}
                            className="px-2.5 py-1 text-[9px] font-black uppercase text-white bg-rose-600 hover:bg-rose-700 rounded-lg shadow transition-all flex items-center gap-1"
                          >
                            <PhoneCall className="w-3 h-3" /> Intercept
                          </button>
                        ) : (
              ...
              <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold mt-0.5">Status: <span className="text-emerald-400">{agentPhase}</span></p>
                  </div>
                </div>
              </div>

              <div className="space-y-4">
                <span className="text-[10px] font-black text-slate-500 tracking-wider">Orchestration Trace Steps:</span>
          ...
          <section className="col-span-1 lg:col-span-7 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex-1">
              <div className="flex gap-2 border-b pb-4 mb-5">
                {[
                  { id: 'console', label: 'HTTP Terminal' },
                  { id: 'prompts', label: 'System Prompts' },
                  { id: 'analytics', label: 'Illustrative Metrics' }
                ].map(tab => {
                  return (
                    <button
                      key={tab.id}
              ...
              {activeTab === 'console' && (
                <div className="space-y-4 h-full flex flex-col">
                  <p className="text-xs text-slate-505 font-bold font-sans">Raw network API stream processed dynamically by Saathi Core Gateway:</p>
                  <div className="bg-slate-900 text-slate-300 rounded-2xl p-5 flex-1 overflow-y-auto font-mono text-xs space-y-3 border border-slate-800 min-h-[300px]">
              ...
              {activeTab === 'prompts' && (
                <div className="space-y-4">
                  <p className="text-xs text-slate-505 font-bold">Raw configuration layers behind multi-agent orchestrator routines:</p>
                  <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
                    {Object.entries(SYSTEM_PROMPTS).map(([name, text]) => (
                      <div key={name} className="p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                        <div className="text-[10px] font-black text-[#4A148C] font-mono uppercase border-b pb-1 mb-1.5">{name}</div>
                        <pre className="text-[9px] font-mono text-slate-600 whitespace-pre-wrap leading-relaxed">{text.trim()}</pre>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'analytics' && (
                <div className="space-y-4">
                  <div className="p-2 bg-amber-50 border border-amber-200 rounded-xl text-[10px] font-black text-amber-800 uppercase text-center tracking-widest">
                    ⚠️ DEMO DATA - ILLUSTRATIVE METRICS ONLY
                  </div>
                  
                  <div className="grid grid-cols-2 gap-3 text-center">
                    <div className="p-3 bg-purple-50/50 rounded-xl border border-purple-100">
                      <span className="text-2xl font-black text-[#4A148C]">{demoMetrics.totalCompleted}</span>
                      <span className="text-[9px] text-slate-400 block uppercase font-bold mt-0.5">Cleared Sessions</span>
                    </div>
                    <div className="p-3 bg-pink-50/50 rounded-xl border border-pink-100 text-center">
                      <span className="text-2xl font-black text-[#EC407A]">{demoMetrics.autoPayAdoptions}</span>
                      <span className="text-[9px] text-slate-400 block uppercase font-bold mt-0.5">AutoPay Adoptions</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
              ...
              <p className="text-xs text-slate-505 leading-relaxed mb-4 font-medium font-sans">
                The companion server features dynamic failovers. If the Node.js API server goes down, Saathi automatically fallbacks safely to local testing.
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Sarvam AI Gateway Key</label>
                  <input
                    type="password"
                    placeholder="srvm_live_..."
                    value={sarvamKey}
                    onChange={(e) => setSarvamKey(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-purple-400 outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Gemini API Key</label>
                  <input
                    type="password"
      ...
      {/* ────────────────────────────────────────────────────────
         MODALS & EXPLANATIONS (Global scope)
         ──────────────────────────────────────────────────────── */}

      {/* Dynamic Recommendation Explanation Modal */}
      {explainModal && lastOutput && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-xs" onClick={() => setExplainModal(false)} />
          <div className="bg-white text-slate-800 rounded-3xl p-6 max-w-md w-full relative z-10 border border-purple-100 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b pb-3">
              <h4 className="text-sm font-black text-[#4A148C] flex items-center gap-1.5">
                <Lightbulb className="w-4 h-4 text-amber-500" /> Explain My Recommendation
              </h4>
              <button onClick={() => setExplainModal(false)} className="text-slate-400 hover:text-slate-850"><X className="w-4 h-4" /></button>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed font-sans">
              We formulated the suggestion <span className="font-bold text-[#EC407A]">"{lastOutput.recommendation}"</span> because you initiated a <span className="font-bold text-[#4A148C]">"{lastOutput.goal}"</span> journey. 
            </p>
            <p className="text-xs text-slate-600 leading-relaxed font-sans">
              By analyzing recurring banking transactions within the current session, the Recommendation Agent computed that enabling this secure, non-disruptive optimization layer mitigates manual oversight and keeps you protected against missed payments.
            </p>
            <button 
              onClick={() => setExplainModal(false)} 
              className="w-full py-2 bg-gradient-to-r from-[#4A148C] to-[#EC407A] text-white text-xs font-bold rounded-xl"
            >
              Got it, Thanks!
            </button>
          </div>
      ...
      {/* Secure Authentication Pin Gate Modal */}
      {authModal.isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-3xl p-6 lg:p-8 shadow-2xl border-[3px] bg-white border-[#4A148C]">
            <div className="text-center space-y-4">
              ...
              <div>
                <h3 className="text-xl font-black text-[#4A148C]">Authorization Required</h3>
                <p className="text-xs text-slate-505 mt-2 font-bold">Enter 6-digit MPIN or use Biometrics to approve.</p>
              </div>
          ...
          <div className="relative z-10 w-full max-w-2xl rounded-3xl p-6 bg-slate-900 text-white border-2 border-[#4A148C] shadow-2xl flex flex-col gap-4 font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3 bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <p className="text-xs uppercase font-black text-slate-400 font-sans">Stream Parameters</p>
              ...
              <div className="space-y-2 flex flex-col justify-center text-center p-4">
                ...
                <p className="text-[11px] text-slate-400 font-sans">Co-pilot has successfully acquired browser state matrices. Administrators may now take terminal control.</p>
              </div>
            </div>

            <div className="p-4 bg-slate-950/50 border border-slate-800 rounded-2xl flex flex-col gap-2">
              <p className="text-[10px] font-black uppercase text-slate-400 font-mono">Co-Pilot Action Terminal</p>
